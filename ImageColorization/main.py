...
if file:
    ...
    try:
        ...
        r = requests.post("https://api.deepai.org/api/colorizer", files={
                          'image': open(file, 'rb'), }, headers={'api-key': key}, verify=False)
        res = r.json()
    ...
    if 'output_url' in res:
        ...
        with open(folder+"/"+"colorized_"+myfile, "wb") as f:
            f.write(c.content)
        a = droid.alert("Image Colorization", "success, save colored image at " +
                        folder+"/"+"colorized_"+myfile, button=("close", "view"))
        if a == "view":
